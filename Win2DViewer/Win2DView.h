      ...
      public CMyScrollView
{
  public:
    ...
    bool IsConsoleDebugEnabled() const noexcept
    {
        return consoleDebugEnabled;
    }
    RenderLayerMode GetRenderLayerMode() const noexcept
    {
        return renderLayerMode;
    }
  ...
  private:
    ...
    wna::wd::uid::DesktopWindowTarget CreateDesktopWindowTarget(wna::wd::uic::Compositor const& compositor, HWND window);
    ...
    void DrawSvgTextOverlay(wna::cv::core::CanvasDrawingSession const& session, wna::wd::num::float3x2 const& transform);

    bool Redraw(float cx, float cy, float wx, float wy, float width, float height, UINT dpi, CRect* clipRect = nullptr);
    void ScenarioWin2D(wna::wd::uic::Compositor const& compositor, wna::wd::uic::ContainerVisual const& root, UINT dpi, int cx, int cy);
    bool LoadSvg();
