...
void DesktopInteropInternal::DesktopHostWindow::InitializeWinRTComposition()
{
    ...
    DesktopInteropNs::wr::check_hresult(interop->CreateDesktopWindowTarget(
        ...
        reinterpret_cast<abi::IDesktopWindowTarget**>(
            DesktopInteropNs::wr::put_abi(compositionTarget))));
    ...
    compositionGraphicsDevice = DesktopInteropNs::mgcu::CanvasComposition::
...
void DesktopInteropInternal::DesktopHostWindow::EnableHostBackdropForWindow()
    const
{
    const BOOL enabled = TRUE;
    (void)::DwmSetWindowAttribute(windowHandle, kDwmAttrUseHostBackdropBrush,
...
void DesktopInteropInternal::DesktopHostWindow::ResizeWinRTSurface()
{
    ...
    if (drawingSurface == nullptr)
    {
        drawingSurface = compositionGraphicsDevice.CreateDrawingSurface(
            DesktopInteropNs::wf::Size(static_cast<float>(width),
...
void DesktopInteropInternal::DesktopHostWindow::DrawWinRTSurface()
{
    ...
    auto drawingSession =
        DesktopInteropNs::mgcu::CanvasComposition::CreateDrawingSession(
            drawingSurface);
    ...
    drawingSession.DrawRectangle(inset, inset, width - (inset * 2.0f),
                                 height - (inset * 2.0f), frameColor, 3.0f);
    ...
    drawingSession.DrawCircle(ballPosition, radius + 6.0f,
                              {0x80, 0xFF, 0xD7, 0x8C}, 2.0f);

    drawingSession.FillRoundedRectangle(
        {24.0f, height - 110.0f, width - 48.0f, 76.0f}, 12.0f, 12.0f, panelColor);
    drawingSession.DrawRoundedRectangle(
        {24.0f, height - 110.0f, width - 48.0f, 76.0f}, 12.0f, 12.0f, panelStroke,
        1.5f);

    const std::wstring title =
        (backend == DesktopInterop::DesktopHostBackend::WinRTHostBackdrop)
    ...
    _snwprintf_s(infoLine, _TRUNCATE,
                 L"Ball(%.1f, %.1f)  Velocity(%.1f, %.1f)  Radius %.1f",
                 ballPosition.x, ballPosition.y, ballVelocity.x, ballVelocity.y,
                 ballRadius);
    drawingSession.DrawText(infoLine, {36.0f, height - 88.0f}, textColor);
}

void DesktopInteropInternal::DesktopHostWindow::UpdateWinRTPhysics(
    float deltaSeconds)
{
    ...
    const float right =
        static_cast<float>(surfacePixelWidth) - 20.0f - ballRadius;
    const float top = 20.0f + ballRadius;
    const float bottom =
        static_cast<float>(surfacePixelHeight) - 20.0f - ballRadius;
...
void DesktopInteropInternal::DesktopHostWindow::ClampWinRTBallPosition()
{
    const float minDimension =
        static_cast<float>((std::min)(surfacePixelWidth, surfacePixelHeight));
    ...
    {
        ballPosition = {static_cast<float>(surfacePixelWidth) * 0.5f,
                        static_cast<float>(surfacePixelHeight) * 0.5f};
